...
for i in np.arange(start, end, step):
    ...
    subprocess.run(
        "gcc Codes/BondCheckerMixed.c -o Codes/BondCheckerMixed -lm", shell=True
    )
    subprocess.run(
        "./Codes/BondCheckerMixed", input="{}".format(i), capture_output=True, text=True
    )

    # Solvation shell calculations
    subprocess.run(
        "g++ Codes/SolvationShellMixed.cpp -o Codes/SolvationShellMixed", shell=True
    )
    subprocess.run("./Codes/SolvationShellMixed", capture_output=True, text=True)

    # Print cumulative H-bond data (for each iteration) for table creation
    subprocess.run("python3 Codes/CompiledHBdataMixed.py", shell=True)

    subprocess.run("python3 Codes/Cycles.py", shell=True)


subprocess.run("python3 Codes/RTadderMixed.py", shell=True)
...
subprocess.run("python3 Codes/TableCreateMixed.py", shell=True)
...
subprocess.run("rm Codes/BondCheckerMixed", shell=True)
subprocess.run("rm Codes/SolvationShellMixed", shell=True)
