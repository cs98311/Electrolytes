...
for i in np.arange(start, end, step):
    ...
    subprocess.run("gcc Codes/BondCheckerZn.c -o Codes/BondCheckerZn -lm", shell=True)
    subprocess.run(
        "./Codes/BondCheckerZn", input="{}".format(i), capture_output=True, text=True
    )

    # Solvation shell calculations
    subprocess.run(
        "g++ Codes/SolvationShellZn.cpp -o Codes/SolvationShellZn", shell=True
    )
    subprocess.run("./Codes/SolvationShellZn", capture_output=True, text=True)
...
subprocess.run(
    "./Codes/Averager",
    input="{}\n{}\nCation/Zn_Ow.txt\nAverages/AvgZnCoodOw.csv".format(n, u),
    capture_output=True,
    text=True,
)
subprocess.run(
    "./Codes/Averager",
    input="{}\n{}\nCation/Zn_Ot.txt\nAverages/AvgZnCoodOt.csv".format(n, u),
    capture_output=True,
    text=True,
)
...
subprocess.run(
    "./Codes/Averager",
    input="{}\n{}\nAnion/Ot_Zn_Coodn.txt\nAverages/AvgOtCoodZn.csv".format(n, t),
    capture_output=True,
    text=True,
)
...
subprocess.run("python3 Codes/TableCreateZn.py", shell=True)
...
def Plot():
    ...
    subprocess.run("python3 Codes/3DplotsZn.py", shell=True)
...
subprocess.run("rm Codes/BondCheckerZn", shell=True)
subprocess.run("rm Codes/SolvationShellZn", shell=True)
...
subprocess.run("rm Cation/Li_Ow.txt", shell=True)
subprocess.run("rm Cation/Li_Ot.txt", shell=True)
subprocess.run("rm Averages/AvgLiCoodOw.csv", shell=True)
subprocess.run("rm Averages/AvgLiCoodOt.csv", shell=True)
subprocess.run("rm Anion/Ot_Li_Coodn.txt", shell=True)
subprocess.run("rm Averages/AvgOtCoodLi.csv", shell=True)
subprocess.run("rm RT/RtimesL.txt", shell=True)
subprocess.run("rm RT/Lrt.txt", shell=True)
