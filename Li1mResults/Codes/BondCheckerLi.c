...
To Run:
gcc Codes/BondCheckerLi.c -o Codes/BondCheckerLi -lm
./Codes/BondCheckerLi
rm Codes/BondCheckerLi
...
int main()
{
	...
	FILE *fOtCompLi = fopen("Anion/OtCompiledLi.txt", "a");

	double t0l0 = 0, t0l1 = 0, t1l0 = 0, t1l1 = 0, tot = 0;

	for (i = 0; i < 4 * numT; i++)
	{
		if (Acceptor_Ot[i] == 0 && Cood_with_Li_Ot[i] == 0)
			t0l0 += 1;
		else if (Acceptor_Ot[i] == 0 && Cood_with_Li_Ot[i] == 1)
			t0l1 += 1;
		else if (Acceptor_Ot[i] == 1 && Cood_with_Li_Ot[i] == 0)
			t1l0 += 1;
		else if (Acceptor_Ot[i] == 1 && Cood_with_Li_Ot[i] == 1)
			t1l1 += 1;
	}
	tot = t0l1 + t0l0 + t1l1 + t1l0;

	fprintf(fOtCompLi, "%.2lf %.2lf %.2lf %.2lf\n", t0l0 * 100 / tot, t0l1 * 100 / tot, t1l0 * 100 / tot, t1l1 * 100 / tot);
	...
	if (numZ != 0)
	{
		...
		fprintf(fZn_Ow, "%.2lf   %.2lf   %.2lf   %.2lf   %.2lf   %.2lf   %.2lf   %.2lf\n", count0 / total_count, count1 / total_count, count2 / total_count, count3 / total_count, count4 / total_count, count5 / total_count, count6 / total_count, total_count);
	}
	...
	if (numZ != 0)
	{
		...
		fprintf(fZn_Ot, "%.2lf   %.2lf   %.2lf   %.2lf   %.2lf   %.2lf   %.2lf   %.2lf\n", count0 / total_count, count1 / total_count, count2 / total_count, count3 / total_count, count4 / total_count, count5 / total_count, count6 / total_count, total_count);
	}
