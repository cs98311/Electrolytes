...
data = np.loadtxt(
    "Plots/DataFiles/Combined.csv", dtype="int", delimiter=",", skiprows=1
)
    ...
    for j in range(0, 2):
        ...
        for k in range(0, 3):
            row_combo = np.append(
                row_combo,
                np.round(
                    np.average((HB_water == i) * (Cood_Li == j) * (HB_tfsi == k)) * 100,
                    2,
                ),
            )
...
with open("Plots/DataFiles/IsoHBabsValues.txt", "ab") as f2:
    np.savetxt(
        f2, row_combo2.reshape(1, row_combo2.shape[0]), fmt="%.2f", delimiter=" "
    )
...
with open("Plots/DataFiles/OneHBabsValues.txt", "ab") as f3:
    np.savetxt(
        f3, row_combo3.reshape(1, row_combo2.shape[0]), fmt="%.2f", delimiter=" "
    )
